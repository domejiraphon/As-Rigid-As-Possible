...
int main(int argc, char *argv[])
{
  ...
  // Load input meshes
  if(!igl::readOBJ(argv[1], V, TC, CN, F, FTC, FN)){
...
S,s      Save current mesh to ./deformed_mesh.obj
⌘ Z      Undo
⌘ ⇧ Z    Redo
)";
  enum Method
    ...
    [&](igl::opengl::glfw::Viewer &, unsigned int key, int mod)
  {
    switch(key)
    {
      ...
      case 's':
      {
        //igl::writeOBJ("deformed_mesh.obj", U, TC, CN, F, FTC, FN);
        igl::writeOBJ("deformed_mesh.obj", U, F, CN, FN, TC, FTC);
        std::cout << "Saved deformed mesh to deformed_mesh.obj." << std::endl;
        break;
      }
      case 'U':
      case 'u':
